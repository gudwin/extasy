 ...
 * 
 * @author Gisma (dmitrey.schevchenko@gmail.com) / d@dd-team.org
...
function startLoading() {
	...
	obj.innerHTML = '<img src="/kernel/pic/layout/loading.gif">';
...
function dTree(objName) {
	...
	this.icon = {
		root				: '/_exst_kernel/pic/submenu/folder.gif',
		folder			: '/_exst_kernel/pic/submenu/folder.gif',
		folderOpen	: '/_exst_kernel/pic/submenu/folderopen.gif',
		node				: '/_exst_kernel/pic/submenu/page.gif',
		empty				: '/_exst_kernel/pic/submenu/empty.gif',
		line				: '/_exst_kernel/pic/submenu/line.gif',
		join				: '/_exst_kernel/pic/submenu/join.gif',
		joinBottom	: '/_exst_kernel/pic/submenu/joinbottom.gif',
		plus				: '/_exst_kernel/pic/submenu/plus.gif',
		plusBottom	: '/_exst_kernel/pic/submenu/plusbottom.gif',
		minus				: '/_exst_kernel/pic/submenu/minus.gif',
		minusBottom	: '/_exst_kernel/pic/submenu/minusbottom.gif',
		nlPlus			: '/_exst_kernel/pic/submenu/nolines_plus.gif',
		nlMinus			: '/_exst_kernel/pic/submenu/nolines_minus.gif'
	};
	this.obj = objName;
	this.aNodes = [];
	this.aIndent = [];
...
(function () {
	var $ = jQuery;
	this.loadTmpl = function loadTmpl( src,id ) {
		var func = function ( src, id ) {
			$.post( src, {}, function ( html ) {
				var template = '<script type="text/html" id="' + id + '">' + html + '</script>'; 
...
cms.szPopupId = '';
cms.httpRoot = '/';
cms.httpCPRoot = '/admin/';
cms.httpKernelPath = '/_exst_kernel/';
 ...
 * 
 */
cms.message = function (subject, content) {
	var autoCloseFnc = function () {
		var autoClose = readCookie('cms_msg_autoclose');
		console.log( autoClose );
		// Вторая проверка, на случай если пользователь убрал галку
		if ( autoClose == 1) {
			tip.remove();
		}
    };
    var createBox = function (t, s){
        ...
        return ['<div class="extcms-msg">',
                '<div class="inner"><h3>', t, '</h3>', s, '</div>',
                '</div>',
                '<div class="autoclose">',
	                '<input type="checkbox" id="cmsMessagesAutoClose"  ' + (autoClose == 1? 'checked="checked"': '' )+'value="1" autocomplete="off">',
	                '<label for="cmsMessagesAutoClose">Закрывать автоматически</label>',
                '</div>',
    ...
    var html = createBox(subject, content)
    var tip = $('<div id="extcms-msg">').append(html);
    ...
    tip.animate({
    	...
    	if ( autoClose ) {
    		window.setTimeout( autoCloseFnc, 5000);
    		
    	}
    });
    // Галочка автозакрытия
    tip.find('.autoclose input').click( function () {
    	var autoClose = this.checked;
    	if ( autoClose ) {
    		window.setTimeout( autoCloseFnc, 5000);
    	}
    	autoClose = autoClose ? 1 : 0;
    	setCookie( 'cms_msg_autoclose', autoClose);
    })
...
jQuery( function ( $ ) {
	...
	$('#launchTests').click( function ( ) { 
		...
		var testDeferred = testUrl( currentTest );;
		var onTestFinish = function ( ) {
			var nextTr = currentTest.next( 'tr' );
			
			if ( nextTr.length > 0 ) {
				currentTest = nextTr;
				testDeferred = testUrl( currentTest );
